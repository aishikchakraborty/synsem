...
class Synsem(nn.module):
    def __init__(self, ntoken, ninp, pos_classes, pretrained_emb):
        ...
        self.W = nn.Parameter(torch.random.rand((ninp, ninp)))

        # POS Losses
        self.linear11 = nn.Linear(ninp//2, 100)
        self.linear12 = nn.Linear(ninp//2, pos_classes)
        self.relu = nn.ReLU()

        self.linear21 = nn.Linear(ninp//2, 100)
        self.linear22 = nn.Linear(ninp//2, pos_classes)
    ...
    def forward(self, x, y):
        ...
        translated_y = torch.bmm(self.W, org_emb_reconstructed_x)
        ...
        recosntruction_loss = F.mse_loss(org_emb_x, org_emb_reconstructed_x) + F.mse_loss(org_emb_y, org_emb_reconstructed_y)
        semantic_loss = F.mse_loss(sem_emb_x, sem_emb_y)

        return translation_loss, recosntruction_loss, semantic_loss, pos_x, pos_y
