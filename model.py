...
class Synsem(nn.Module):
    ...
    def forward(self, x, y):
        ...
        sem_emb_x = torch.mm(src_emb, self.W)
        syn_emb_x = src_emb - sem_emb_x

        tgt_emb = self.tgt_emb(y)
        sem_emb_y = torch.mm(tgt_emb, self.W)
        syn_emb_y = tgt_emb - sem_emb_y

        pos_x = self.linear12(self.relu(self.linear11(syn_emb_x)))
        pos_y = self.linear22(self.relu(self.linear21(syn_emb_y)))

        translation_loss  = self.cos(tgt_emb, tgt_emb)
        reconstruction_loss = F.mse_loss(src_emb, src_emb) + F.mse_loss(tgt_emb, tgt_emb)
